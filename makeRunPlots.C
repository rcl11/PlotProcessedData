...
void CreateRunPlots( const std::vector<std::string>& files, bool ntuple=true )
{
  ...
  for(unsigned i=0; i<files.size(); ++i){
      ...
      TH1D* hposx = new TH1D( "hposx", "hposx", 100, 0.0, 1000.0 );
      TH1D* hposy = new TH1D( "hposy", "hposy", 100, 0.0, 1000.0 );
      TH1D* hposz = new TH1D( "hposz", "hposz", 100, 0.0, 1000.0 );
      TH1D* hposR = new TH1D( "hposR", "hposR", 100, 0.0, 1000.0 );
      ...
      TH1D* hrpmt = new TH1D( "hrpmt", "hrpmt", 1000, 0.0, 10000.0 );
      ...
      if(ntuple) {
          ...
          for (Long64_t j=0;j<nentries;j++) {
             ...
             if(j==0){
                 ...
                 std::cout << dataclean << " " << compatibility_cut << std::endl;
             }
          ...
          for( size_t iEntry = 0; iEntry < dsReader.GetEntryCount(); iEntry++ )
          {
              ...
              {   
                  ...
                  if(iEntry==0){
                      std::cout << "first event" << std::endl;
                      std::cout << RAT::EventIsClean( rEV, rMeta, rDataCleaningWord ) << std::endl;
                  }
      ...
      //CHeck this!!
      hNEvents_vs_run->SetBinError(i+1,sqrt(n_events)/n_events);
      ...
      c5->Update();
      c5->SaveAs(("posx_"+outname+postfix+".png").c_str());
      c5->SaveAs(("posx_"+outname+postfix+".pdf").c_str());
      c5->Clear();
      ...
      c6->Update();
      c6->SaveAs(("posy_"+outname+postfix+".png").c_str());
      c6->SaveAs(("posy_"+outname+postfix+".pdf").c_str());
      c6->Clear();
      ...
      c7->Update();
      c7->SaveAs(("posz_"+outname+postfix+".png").c_str());
      c7->SaveAs(("posz_"+outname+postfix+".pdf").c_str());
      c7->Clear();
      ...
      c8->Update();
      c8->SaveAs(("posR_"+outname+postfix+".png").c_str());
      c8->SaveAs(("posR_"+outname+postfix+".pdf").c_str());
      c8->Clear();
      ...
      c9->Update();
      c9->SaveAs(("rpmt_"+outname+postfix+".png").c_str());
      c9->SaveAs(("rpmt_"+outname+postfix+".pdf").c_str());
      c9->Clear();
  ...
  c12->Update();
  c12->SaveAs(("nevents_vs_run_"+start_run+"_to_"+end_run+postfix+".png").c_str());
  c12->SaveAs(("nevents_vs_run_"+start_run+"_to_"+end_run+postfix+".pdf").c_str());
  delete c10;
  delete c11;
  delete c12;
